...
void main()
{
	...
	vec4 t1 = UV.xxxy + vec4( -dx, 0.0, dx,-2.0*dy); // A1 B1 C1
	vec4 t2 = UV.xxxy + vec4( -dx, 0.0, dx, -dy);    //  A  B  C
	vec4 t3 = UV.xxxy + vec4( -dx, 0.0, dx, 0.0);    //  D  E  F
	vec4 t4 = UV.xxxy + vec4( -dx, 0.0, dx, dy);     //  G  H  I
	vec4 t5 = UV.xxxy + vec4( -dx, 0.0, dx, 2.0*dy); // G5 H5 I5
	vec4 t6 = UV.xyyy + vec4(-2.0*dx,-dy, 0.0, dy);  // A0 D0 G0
	vec4 t7 = UV.xyyy + vec4( 2.0*dx,-dy, 0.0, dy); // C4 F4 I4
	vec2 f = fract(UV * vec2(160, 144));
